...
public interface TraceFunTemplateconfigMapper extends CommonSql{
	static String allFields="Id id,TraceTemplateId traceTemplateId,NodeWeight nodeWeight,"
			              + "BusinessType businessType,NodeFunctionId nodeFunctionId,NodeFunctionName nodeFunctionName,"
			              + "CreateTime createTime,UpdateTime updateTime";
	
	static String allFieldsWithPrefix="t.Id id,t.TraceTemplateId traceTemplateId,t.NodeWeight nodeWeight,"
            + "t.BusinessType businessType,t.NodeFunctionId nodeFunctionId,t.NodeFunctionName nodeFunctionName,"
	...
	@Insert(startScript
			...
			+ " <trim prefix='(' suffix=')' suffixOverrides=','>"
				...
				+ "<if test='nodeWeight != null'>NodeWeight,</if> "
	...
	@Select("select "+allFieldsWithPrefix+",ts.TraceTemplateName from trace_fun_templateconfig t left join trace_funtemplatestatistical ts on t.TraceTemplateId=ts.TraceTemplateId where t.TraceTemplateId=#{templateConfigId} order by t.NodeWeight")
	...
	@Update("<script>"+
			"update trace_fun_templateconfig"
				+ "<set>"
					...
					+ "<if test='nodeWeight != null'>NodeWeight=#{nodeWeight},</if> "
					+"UpdateTime=now()"
				+ "</set>"
				+ "where Id=#{id}"
				+" </script>"
	...
	@Select(" <script>"
			...
			+ "  GROUP BY trace_funtemplatestatistical.TraceTemplateId  order by trace_funtemplatestatistical.CreateTime desc"
	...
	@Select("SELECT Id,NodeFunctionName FROM trace_fun_templateconfig WHERE NodeFunctionId= #{nodeFunctionId} ")
	List<TraceFunTemplateconfig> selectByFunId(@Param("nodeFunctionId")String nodeFunctionId);
